...
1. 点击「选择图片」按钮
2. 从本地选择一张或多张图片(支持多选)
3. 系统将自动并行生成缩略图（保持原图比例，最大边长 400px）
4. 处理完成后会在页面顶部**常驻显示**统计信息卡片，包括：
   - 📊 共处理图片数量
   - ⏱️ 总耗时（毫秒）
   - ⚡ 平均每张耗时
   - 🚀 并行加速比
5. 查看每张图片的原图/缩略图对比和单张处理时间
6. 可点击统计卡片右上角的 ✕ 关闭统计信息
7. 可继续添加更多图片，或点击「清空所有」重新开始
...
- **渐进式渲染** - 每张图片处理完成后立即显示，无需等待全部完成
- **完整统计** - 常驻显示总耗时、平均耗时、加速比等详细性能数据
- **可关闭提示** - 统计卡片右上角有关闭按钮，可随时关闭
